...
class Test_simulate_eses(unittest.TestCase):
    ...
    def test_generate_motifs_diff_lengths(self):
        motif_set = gen.read_many_fields("test_data/test_generate_motifs_diff_lengths/motif_set.txt", ",")
        motif_set = [i[0] for i in motif_set]
        dinucleotides = gen.read_many_fields("test_data/test_generate_motifs_diff_lengths/dinucleotides.txt", ",")
        dinucleotides = [i[0] for i in dinucleotides]
        seed_list = [5]
        expected = gen.read_many_fields("test_data/test_generate_motifs_diff_lengths/expected_simulated_motifs.txt", ",")
        expected = [i[0] for i in expected]
        observed = se.generate_motifs([0], motif_set, dinucleotides, seed=seed_list)[0]
        self.assertEqual(expected, observed)

    def test_generate_motifs_sets(self):
        motif_set = gen.read_many_fields("test_data/test_generate_motifs_sets/motif_set.txt", ",")
        motif_set = [i[0] for i in motif_set]
        seed_list = range(3)
        expected = "test_data/test_generate_motifs_sets/expected_simulated_motifs.txt"
        observed = "test_data/test_generate_motifs_sets/observed_simulated_motifs.txt"
    ...
    def test_get_stop_codon_count(self):
        motif_set = gen.read_many_fields("test_data/test_get_stop_codon_count/motif_set.txt", ",")
        motif_set = [i[0] for i in motif_set]
        expected = 8
